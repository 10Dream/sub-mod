...
# تنظیم مهلت زمانی کوتاه برای جلوگیری از قفل شدن یا معطلی در رزولوشن DNS دامنه‌ها
...
def log_stage(step: int, total: int, message: str):
    ...
    if step == 1:
        print(f"\n{yellow}┌────────────────────────────────────────────────────────┐{reset}", flush=True)
        print(f"{yellow}│     شروع فرآیند مگا-میکس، فیلترینگ و اسپلیت روزانه    │{reset}", flush=True)
        print(f"{yellow}└────────────────────────────────────────────────────────┘{reset}\n", flush=True)
        
    print(f"{green}{bold}[{step}/{total}]{reset} {cyan}{bar}{reset} {bold}{percent}%{reset} | {message}", flush=True)
    
    if step == total:
        print(f"\n{green}┌────────────────────────────────────────────────────────┐{reset}", flush=True)
        print(f"{green}│        فرآیند اسپلیت روزانه با موفقیت به پایان رسید!  │{reset}", flush=True)
        print(f"{green}└────────────────────────────────────────────────────────┘{reset}\n", flush=True)
...
def resolve_ip(server: str):
    ...
    if ":" in server_clean:
        try:
            ...
            DNS_CACHE[server_clean] = (server_clean, "ipv6")
    ...
    try:
        ...
        DNS_CACHE[server_clean] = (server_clean, "ipv4")
    ...
    try:
        ...
        for family, _, _, _, sockaddr in addr_info:
            ...
            if family == socket.AF_INET:
                DNS_CACHE[server_clean] = (ip, "ipv4")
                return ip, "ipv4"
            elif family == socket.AF_INET6:
                DNS_CACHE[server_clean] = (ip, "ipv6")
                return ip, "ipv6"
    except Exception:
        pass
        
    DNS_CACHE[server_clean] = (None, "unknown")
    return None, "unknown"
...
def build_clash_config(proxies_list, dest_yaml_path, limit=800):
    if not proxies_list:
        return False
        
    if limit and limit > 0:
        target_proxies = proxies_list[:limit]
    else:
        target_proxies = proxies_list
        
    processed_proxies = []
    seen_names = {}
    
    for p in target_proxies:
        p_copy = dict(p)
...
def main():
    ...
    log_stage(8, total_stages, "رندر و ذخیره همزمان فایل‌های معمولی، کدهای بیس۶۴ و کانفیگ‌های دوگانه (محدود و نامحدود) کلش...")
    ...
    for cat_name, data in categories.items():
        ...
        # نسخه محدود به ۸۰۰ کانفیگ
        build_clash_config(dicts, os.path.join(split_clash_dir, f"{cat_name}.yaml"), limit=800)
        # نسخه نامحدود
        build_clash_config(dicts, os.path.join(split_clash_dir, f"{cat_name}_unlimited.yaml"), limit=None)
    ...
    for cc, data in countries_data.items():
        ...
        with open(os.path.join(countries_base64_dir, f"{cc}.txt"), 'w', encoding='utf-8') as f:
            f.write(encode_to_base64(content_str))
        # نسخه محدود به ۸۰۰ کانفیگ
        build_clash_config(dicts, os.path.join(countries_clash_dir, f"{cc}.yaml"), limit=800)
        # نسخه نامحدود
        build_clash_config(dicts, os.path.join(countries_clash_dir, f"{cc}_unlimited.yaml"), limit=None)
    ...
    for dc_raw_name, data in datacenters_data.items():
        ...
        with open(os.path.join(datacenters_base64_dir, f"{dc_filename}.txt"), 'w', encoding='utf-8') as f:
            f.write(encode_to_base64(content_str))
        # نسخه محدود به ۸۰۰ کانفیگ
        build_clash_config(dicts, os.path.join(datacenters_clash_dir, f"{dc_filename}.yaml"), limit=800)
        # نسخه نامحدود
        build_clash_config(dicts, os.path.join(datacenters_clash_dir, f"{dc_filename}_unlimited.yaml"), limit=None)
    ...
    for proto, data in protocols_data.items():
        ...
        with open(os.path.join(protocols_base64_dir, f"{proto}.txt"), 'w', encoding='utf-8') as f:
            f.write(encode_to_base64(content_str))
        # نسخه محدود به ۸۰۰ کانفیگ
        build_clash_config(dicts, os.path.join(protocols_clash_dir, f"{proto}.yaml"), limit=800)
        # نسخه نامحدود
        build_clash_config(dicts, os.path.join(protocols_clash_dir, f"{proto}_unlimited.yaml"), limit=None)
