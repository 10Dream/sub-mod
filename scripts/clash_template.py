# =====================================================================
# ماژول تمپلت پیکربندی حرفه‌ای و ضد فیلترینگ کلش میهومو (Modular Clash/Mihomo Template)
# منطبق بر آخرین استانداردهای دور زدن فیلترینگ شدید و بهینه‌سازی ترافیک در ایران
# =====================================================================

# ۱. تنظیمات عمومی و ضد فریز شبکه
GENERAL_SETTINGS = {
    "global-client-fingerprint": "chrome",
    "keep-alive-interval": 5,     # تست کانکشن هر ۵ ثانیه برای کشف سریع قطعی‌ها
    "keep-alive-idle": 10,        # شروع بررسی بعد از ۱۰ ثانیه بی‌تحرکی کانکشن
    "disable-keep-alive": False,  # حتماً روشن باشد تا کانکشن‌ها در ایران فریز نشوند
    "port": 7890,
    "socks-port": 7891,
    "redir-port": 7892,
    "mixed-port": 7893,
    "tproxy-port": 7894,
    "allow-lan": True,
    "tcp-concurrent": True,       # تست هم‌زمان چند مسیر برای اتصال سریع‌تر
    "enable-process": True,
    "find-process-mode": "always",
    "ipv6": False,                # خاموش به دلیل اختلال شدید در ایران [9]
    "log-level": "info",          # بهینه برای کاهش مصرف باتری و پردازنده [9]
...
# ۲. تنظیمات پیشرفته و ضد فیلترینگ DNS (اجبار به استفاده از پروتکل TCP جهت پایداری ۱۰۰٪ در ایران)
DNS_SETTINGS = {
    "enable": True,
    "ipv6": False,               # غیرفعال‌سازی برای جلوگیری از تایم‌اوت [9]
    "prefer-h3": False,          # غیرفعال‌سازی HTTP/3 به دلیل فیلترینگ شدید روی پروتکل کوئیک [9]
    "cache-algorithm": "arc",
    ...
    # دی‌ان‌اس‌های اولیه برای پیدا کردن آدرس سرورها [9]
    "default-nameserver": [
        "1.1.1.1",
        "8.8.8.8",
        "9.9.9.9",
    ],
    # دی‌ان‌اس‌های اصلی (اجبار به TCP برای فرار از دراپ شدن ترافیک UDP در فایروال ایران) [9]
    "nameserver": [
        "tcp://1.1.1.1",
        "tcp://1.0.0.1",
        "tcp://8.8.8.8",
    ...
    # شبکه عظیم دی‌ان‌اس بکاپ (TCP شده جهت پایداری اتصال) [9]
    "fallback": [
        "tcp://76.76.19.19",
        "tcp://76.223.122.150",
        "tcp://185.228.168.9",
    ...
    # فیلتر هوشمند برای سوئیچ آنی به بکاپ‌ها در صورت آلودگی و مسمومیت دی‌ان‌اس در ایران [9]
    "fallback-filter": {
        "geoip": True,
        "geoip-code": "IR",
        "geosite": ["gfw"],
    ...
    # دی‌ان‌اس اختصاصی برای حل آدرس سرورهای خود پروکسی
...
# ۳. سیستم اسنیفر (شنود و تشخیص صحیح پروتکل‌ها جهت فیلترشکن هوشمند)
SNIFFER_SETTINGS = {
    "enable": True,
...
# ۴. تنظیمات تونل عمیق شبکه مجازی TUN
TUN_SETTINGS = {
    "enable": True,
    "stack": "mixed",
    "auto-route": True,
...
# ۵. لیست کامل مخازن قوانین (Rule Providers) بهینه‌سازی شده در ایران [9]
RULE_PROVIDERS = {
...
# ۶. قوانین روتینگ و هدایت ترافیک (Routing Rules) [9]
RULES = [
...
def get_proxy_groups(proxy_names_list):
    """
    تولید لیست کامل گروه‌های پروکسی با ساختار درخواستی و اعمال پویای پروکسی‌ها به فیلدهای مربوطه.
    کدهای مستقیم و روتینگ‌ها به بهینه‌ترین حالت متصل شده‌اند [9].
    """
    # در صورتی که دیتایی واکشی نشده بود، کلش با یک متغیر پیش‌فرض DIRECT ارور نخواهد داد
    proxies_safe = proxy_names_list if proxy_names_list else ["DIRECT"]
...
# ۸. پروتکل بازیابی زمان محلی (NTP)
NTP_SETTINGS = {
    "enable": True,
